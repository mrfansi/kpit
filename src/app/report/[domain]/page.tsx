...
export default async function ReportPage({ params, searchParams }: Props) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground p-8 max-w-4xl mx-auto print:p-0 print:max-w-full print:bg-white print:text-black">
      {/* Header */}
      <header className="flex items-start justify-between border-b-2 border-foreground pb-4 mb-6">
        <div className="flex items-center gap-3">
          <Icon className="w-8 h-8" style={{ color: domain.color }} />
          <div>
            <h1 className="text-2xl font-bold">KPI Scorecard — {domain.name}</h1>
            <p className="text-sm text-muted-foreground">{domain.description ?? "Laporan performa KPI"}</p>
          </div>
        </div>
      ...
      <div className="mt-8 flex gap-3 print:hidden">
        ...
        <a
          ...
          className="px-4 py-2 border text-sm rounded hover:bg-muted transition-colors"
        >
          &larr; Kembali
        </a>
      </div>
